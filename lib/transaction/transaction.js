...
/**
 * Retrieve a hexa string that can be used with aidpd's CLI interface
 ...
 * * `disableMoreOutputThanInput`: disable checking if the transaction spends more aidpcoins than the sum of the input amounts
 * @return {string}
 */
 ...
 *
 * Can receive, as output information, the output of aidpd's `listunspent` command,
 * and a slightly fancier format recognized by aidpcore:
 *
 * ```
 * {
 ...
 * }
 * ```
 * Where `address` can be either a string or a aidpcore Address object. The
 * same is true for `script`, which can be a string or a aidpcore Script.
 *
 ...
 *
 * // From a pay to public key hash output from aidpd's listunspent
 ...
 * CheckTransaction in aidpcoin core.
 */
...
/**
 * Analogous to aidpd's IsCoinBase function in transaction.h
 */

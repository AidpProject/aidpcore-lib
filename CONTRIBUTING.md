Contributing to Aidpcore
=======

We're working hard to make *aidpcore* the most powerful JavaScript library for working with Aidpcoin. Our goal is to have *aidpcore* be a library that can be used by anyone interested in Aidpcoin, and to level expertise differences with great design and documentation.
...
These are some global design goals in aidpcore that any change must adhere.
...
When possible, aidpcore objects should have standard methods on an instance prototype:
...
`JSON.stringify` and `JSON.parse` are expected to be handled outside of the scope of Aidpcore methods. For example, calling `JSON.stringify` on a Aidpcore object will behave as expected and call `transaction.toJSON()` and then stringify it:
...
#### T3 - Require 'aidpcore' and Look up Classes from There

This helps to make tests more useful as examples, and more independent of where they are placed. This also helps prevent forgetting to include all submodules in the aidpcore object.

DO:
```javascript
var aidpcore = require('../');
var PublicKey = aidpcore.PublicKey;
```
DON'T:
```javascript
...
```sh
git remote add AidpProject git@github.com:AidpProject/aidpcore.git
git pull --rebase AidpProject master
```
...
```
Finally go to [github.com/AidpProject/aidpcore](https://github.com/AidpProject/aidpcore) in your web browser and issue a new pull request.

Main contributors will review your code and possibly ask for changes before your code is pulled in to the main repository.  We'll check that all tests pass, review the coding style, and check for general code correctness. If everything is OK, we'll merge your pull request and your code will be part of aidpcore.

If you have any questions feel free to post them to
[github.com/AidpProject/aidpcore/issues](https://github.com/AidpProject/aidpcore/issues).
